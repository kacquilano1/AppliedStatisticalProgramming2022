...
wordcloud(words = TDMTweets$word, freq = TDMTweets$freq, min.freq = 3, max.words = 50, random.order = TRUE, random.color = TRUE, rot.per = .25, colors = kovesi.rainbow(50))
#save the word-cloud to the repository
dev.off()

#Create a document term matrix
DTM <- DocumentTermMatrix(TweetCorpus, control = list(weighting = weightTfIdf))
#obtain 50 terms with highest tf.idf with lower frequency bound of .8
DTM %>% 
  #find terms with lower frequency bound of .8
  findFreqTerms(lowfreq = .8) %>% 
